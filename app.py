...
@app.route("/solve_bestfirst", methods=["POST"])
def solve_bestfirst_route():
    """
    Receive the puzzle from the frontend, solve it with Greedy Best-First Search,
    and return the solution steps + all visited states as JSON.
    """
    data = request.get_json()
    start_state = data.get("state")
    ...
    solution_path, steps_explored, visited_states = solve_best_first(start_state)
    ...
    return jsonify({
        "success":        True,
        "solution":       solution_path,
        "total_moves":    len(solution_path),      # Path cost (not guaranteed optimal)
